...
# 데이터 로드 및 전처리 (다중 인코딩 시도)
@st.cache_data
def load_data(file_path):
    """
    CSV 파일을 로드하고 필요한 전처리를 수행합니다.
    여러 인코딩을 순서대로 시도하여 로드 오류를 해결합니다.
    """
    # Excel에서 저장된 파일에 흔한 인코딩을 순서대로 시도합니다.
    ENCODINGS = ['cp1252', 'latin-1', 'ISO-8859-1', 'utf-8']
    df = None
    
    for encoding in ENCODINGS:
        try:
            # 지정된 인코딩으로 파일 로드를 시도합니다.
            df = pd.read_csv(file_path, encoding=encoding)
            st.success(f"데이터를 '{encoding}' 인코딩으로 성공적으로 로드했습니다.")
            break  # 로드에 성공하면 반복을 중단합니다.
        except UnicodeDecodeError:
            # 디코딩 오류가 발생하면 다음 인코딩을 시도합니다.
            continue
        except Exception as e:
            # 파일 경로 오류 등 인코딩과 무관한 오류가 발생하면 즉시 보고합니다.
            st.error(f"데이터 로드 중 예상치 못한 오류가 발생했습니다: {e}")
            return None

    if df is None:
        st.error("💔 로드 실패: 시도한 모든 인코딩(cp1252, latin-1, ISO-8859-1, utf-8)으로 파일을 읽을 수 없습니다.")
        st.error("해결책: 데이터 파일을 메모장이나 VS Code로 열어 **UTF-8 인코딩**으로 '다른 이름으로 저장'하여 변환해 주십시오.")
        return None

    # --- 데이터 전처리 시작 (로드가 성공했을 경우) ---
